...
while(<IN>){
	...
	if($chr1 eq $chr2){
		...
		if($e1 >= $s2){
			$line="perl $Bin/Stat_share.pl $bam $mapq $bin $chr1 $s1 $e2 0 $out/$name 0 $cutoff $type > $out/$name.log 2>&1";
		}else{
			$line="perl $Bin/Stat_share_dif.pl $bam $mapq $bin $chr1 $s1 $e1 $chr2 $s2 $e2 0 $out/$name 0 $cutoff $type > $out/$name.log 2>&1";
		}
	}else{
		my $s1=$pos1-$ext;
		my $e1=$pos1+$ext;
